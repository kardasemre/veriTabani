...
$etkinliklerOturumlar = $userView->listKatilimciEtkinlikBilgisi();

// ID'ye göre etkinlik oturum bilgisini alalım ve ekrana yazdıralım
if (isset($_POST['id'])) {
    $id = $_POST['id'];
    $etkinlikOturum = $userView->listKatilimciEtkinlikBilgisiById($id);
}
...
if (isset($_POST['search'])) {
    $search = $_POST['search'];
    if (!empty($search)) {
        $etkinliklerOturumlar = $userView->searchKatilimciEtkinlikBilgisi($search);
...
<head>
    ...
    <title>Katılımcı Stantlar</title>
    <style>
        body {
...
<body>
    ...
    <h1>Katılımcı Stantları</h1>
    
    <!-- Arama Formu -->
    <form method="POST" action="">
        <label for="search">Katılımcı ID ile Ara:</label>
